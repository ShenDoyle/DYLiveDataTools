...
// @downloadURL  https://gitee.com/cvvik/DYLiveDataTools/raw/main/DYLiveDataTools.user.js
// @updateURL    https://gitee.com/cvvik/DYLiveDataTools/raw/main/DYLiveDataTools.user.js
// ==/UserScript==
...
(function() {
    ...
    const STATE = {
        ...
        isOnCrmPage: window.location.href.startsWith(CONFIG.crmPageURL),
        isOnDouyinDomain: window.location.hostname.endsWith('douyin.com'),
        currentData: [],
    };

    // --- UI ELEMENTS: 页面元素的创建与管理 ---
    const UI = {
        widgetWrapper: null,
        mainIcon: null,
        panel: null,
        ...
        createWidget() {
            this.widgetWrapper = document.createElement('div'); this.widgetWrapper.id = 'dk-widget-wrapper';
            this.mainIcon = document.createElement('div'); this.mainIcon.id = 'dk-main-icon'; this.mainIcon.style.backgroundImage = `url("${CONFIG.svgIcon}")`;
            this.widgetWrapper.appendChild(this.mainIcon);
        ...
        createPanel() {
            ...
            this.panel = document.getElementById('dk-data-panel');
        }
    };
    ...
    const Core = {
        ...
        refreshData() {
            if (STATE.isOnReviewPage) {
                STATE.currentData = Core.dataPaths.map(item => {
                    const rawValue = Core.getValueByXpath(item.xpath);
                    const processedValue = item.process ? item.process(rawValue) : rawValue;
                    return { label: item.label, value: processedValue };
                });
                GM_setValue('dyLiveData', JSON.stringify(STATE.currentData));
            } else {
                STATE.currentData = JSON.parse(GM_getValue('dyLiveData', '[]'));
            }
            Core.renderPanel(STATE.currentData);
        },

        // 渲染数据到面板
        renderPanel(data) {
            const contentDiv = document.getElementById('dk-data-panel-content');
    ...
    const SmartTooltip = {
        ...
        init(toggleInput) {
            ...
            document.addEventListener('click', (e) => { if (this.tooltip && this.tooltip.classList.contains('visible') && !this.tooltip.contains(e.target) && e.target !== this.activeInput) { this.hide(); } }, true);
            document.addEventListener('keydown', (e) => { if (e.key === 'Escape') this.hide(); });
        },
        activate() { this.isActive = true; this.toggleInput.checked = true; },
        deactivate() { this.isActive = false; this.toggleInput.checked = false; this.hide(); },
    ...
    // --- INITIALIZATION: 脚本启动与事件绑定 ---
    function initialize() {
        if (document.getElementById('dk-widget-wrapper')) return; // 防止重复初始化

        // 1. 创建UI元素
        UI.init();

        // 2. 绑定通用拖动逻辑
        const { widgetWrapper, mainIcon } = UI;
        ...
        if (STATE.isOnReviewPage || STATE.isOnCrmPage) {
            ...
            document.getElementById('dk-copy-all-btn').addEventListener('click', () => { if (STATE.currentData.length > 0) { const allDataText = STATE.currentData.map(item => `${item.label}：${item.value}`).join('\n'); GM_setClipboard(allDataText, 'text'); event.target.textContent = '已全部复制!'; setTimeout(() => { event.target.textContent = '复制全部数据'; }, 2000); } });
