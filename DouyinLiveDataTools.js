...
(function() {
    ...
    const isOnDouyinDomain = window.location.hostname.endsWith('douyin.com');
    ...
    const widgetWrapper = document.createElement('div'); widgetWrapper.id = 'dk-widget-wrapper';
    const mainIcon = document.createElement('div'); mainIcon.id = 'dk-main-icon'; mainIcon.style.backgroundImage = `url("${encodedSvg}")`;
    widgetWrapper.appendChild(mainIcon);
    ...
    if (isOnReviewPage || isOnCrmPage) {
        // --- 3.1. 在抖音复盘或CRM页面: 创建数据面板并定义其所有功能 ---
        const processDuration = (text) => {
            if (!text) return '0.0';
        ...
        const renderPanel = (data) => { contentDiv.innerHTML = ''; data.forEach(item => { const rowHTML = `<div class="dk-data-row"><span class="dk-data-label">${item.label}</span><div class="dk-data-value-wrapper"><span class="dk-data-value">${item.value}</span><button class="dk-copy-btn" data-value="${item.value}">复制</button></div></div>`; contentDiv.insertAdjacentHTML('beforeend', rowHTML); }); contentDiv.querySelectorAll('.dk-copy-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); GM_setClipboard(btn.dataset.value, 'text'); btn.textContent = '已复制!'; setTimeout(() => { btn.textContent = '复制'; }, 1500); }); }); };
        const refreshData = () => { if (isOnReviewPage) { currentData = dataPaths.map(item => { const rawValue = getValueByXpath(item.xpath); const processedValue = item.process ? item.process(rawValue) : rawValue; return { label: item.label, value: processedValue }; }); GM_setValue('dyLiveData', JSON.stringify(currentData)); } else { currentData = JSON.parse(GM_getValue('dyLiveData', '[]')); } renderPanel(currentData); };
        closeBtn.addEventListener('click', () => panel.classList.remove('visible'));
        refreshBtn.addEventListener('click', refreshData);
        copyAllBtn.addEventListener('click', () => { if (currentData.length > 0) { const allDataText = currentData.map(item => `${item.label}：${item.value}`).join('\n'); GM_setClipboard(allDataText, 'text'); copyAllBtn.textContent = '已全部复制!'; setTimeout(() => { copyAllBtn.textContent = '复制全部数据'; }, 2000); } });
        ...
        if (isOnCrmPage) {
            ...
            const SmartTooltip = {
                ...
                isActive: false,
                init() { this.dataMap = new Map(JSON.parse(GM_getValue('dyLiveData', '[]')).map(i => [i.label, i.value])); document.addEventListener('focusin', this.handleFocusIn.bind(this)); document.addEventListener('click', (e) => { if (this.tooltip && this.tooltip.classList.contains('visible') && !this.tooltip.contains(e.target) && e.target !== this.activeInput) { this.hide(); } }, true); document.addEventListener('keydown', (e) => { if (e.key === 'Escape') this.hide(); }); },
                activate() { this.isActive = true; smartFillToggleInput.checked = true; }, deactivate() { this.isActive = false; smartFillToggleInput.checked = false; this.hide(); },
        ...
        mainIcon.addEventListener('click', (e) => { if (wasDragged) { e.stopPropagation(); return; } const isVisible = panel.classList.toggle('visible'); if (isVisible) { refreshData(); } });
    } else if (isOnDouyinDomain) {
        // --- 3.2. 在其他抖音页面: 只提供跳转功能 ---
        mainIcon.addEventListener('click', (e) => { if (wasDragged) { e.stopPropagation(); return; } GM_openInTab(reviewPageURL, { active: true }); });
    }
})();