...
// @version      0.2
// @description  为蝶云直播设计的浏览器助手。核心功能：1) 提供一个可自由拖动的全局快捷入口。2) 在复盘页，智能提取核心数据并生成可拖动的数据面板，支持自动及手动刷新。3) 支持单项或全部数据的一键复制，大幅提升数据整理效率。
// @author       左立方
...
(function() {
    ...
    if (isOnReviewPage) {
        const dataPaths = [
            ...
            { label: '直播日期', xpath: '/html/body/div[1]/div/div[3]/div/div/div/div/div[2]/div/div/div/div/div[2]/div[1]/div/div[1]/div/div/div[2]/div[1]/span[2]', process: (text) => text ? text.split(' ')[0] : '未找到' },
        ...
        const panelHTML = `
            <div id="dk-data-panel">
                <div id="dk-data-panel-header">
                    <span class="header-title">直播数据分析</span>
                    <div class="header-buttons">
                        <span id="dk-data-panel-refresh" title="刷新"></span>
                        <span id="dk-data-panel-close" title="关闭"></span>
                    </div>
                </div>
                <div id="dk-data-panel-content"></div>
                <div id="dk-data-panel-footer"><button id="dk-copy-all-btn">复制全部数据</button></div>
            </div>`;
        ...
        copyAllBtn.addEventListener('click', () => { const allDataText = dataPaths.map(item => { let rawValue = getValueByXpath(item.xpath); let processedValue = item.process ? item.process(rawValue) : rawValue; return `${item.label}：${processedValue}`; }).join('\n'); GM_setClipboard(allDataText, 'text'); copyAllBtn.textContent = '已全部复制!'; setTimeout(() => { copyAllBtn.textContent = '复制全部数据'; }, 2000); });

        refreshData = function() { contentDiv.innerHTML = ''; dataPaths.forEach(item => { let rawValue = getValueByXpath(item.xpath); let processedValue = item.process ? item.process(rawValue) : rawValue; const rowHTML = `<div class="dk-data-row"><span class="dk-data-label">${item.label}</span><div class="dk-data-value-wrapper"><span class="dk-data-value">${processedValue}</span><button class="dk-copy-btn" data-value="${processedValue}">复制</button></div></div>`; contentDiv.insertAdjacentHTML('beforeend', rowHTML); }); contentDiv.querySelectorAll('.dk-copy-btn').forEach(btn => { btn.addEventListener('click', (e) => { e.stopPropagation(); GM_setClipboard(btn.dataset.value, 'text'); btn.textContent = '已复制!'; setTimeout(() => { btn.textContent = '复制'; }, 1500); }); }); };
        ...
        if (targetNode) {
            ...
            console.log('蝶云科技抖音直播数据助手：未找到用于实时更新的监视目标。');
        }
    }
})();